$(document).ready(() => {
    $('.xo-game').click(()=>{
        ...
        //  Display game
        $('.game-display').html('');
        $('.game-display').html('<div class="winner"></div><div class="game-time start-button">Start</div><div style="padding-top:20px; width:'+ 50*b_sz +'px; height:'+ 50*b_sz +'px; display:flex; flex-flow:row wrap;" class="xo-game-board"></div>');
        for(let i=1; i<=b_sz*b_sz; i++){
            $('.xo-game-board').append('<div class="xo-square" id="xo-' + i + '"></div>')
        }
        //  Function
        ...
        function checkWin(index){
            if(checkRow(index, board[index]) === 1 || checkColumn(index, board[index]) === 1 || checkCross(index, board[index]) === 1){
