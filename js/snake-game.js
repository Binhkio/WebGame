$(document).ready(()=>{
    $('.snake-game').click(()=>{
        ...
        //  Display game
        $('.game-display').html('')
        $('.game-display').html('<div class="score">Score: 0</div><div class="game-time start-button">Start</div><div style="width:'+ (10*boardSize+5) +'px; height:'+ (10*boardSize+5) +'px; display:flex; flex-flow:row wrap;" class="snake-game-board"></div>');
        for(let i=1; i<=boardSize*boardSize; i++){
            $('.snake-game-board').append('<div class="snake-square" id="snake-' + i + '"></div>')
        }
        ...
        function moveBody(){
            ...
            if(curMove === 119 || curMove === 87){   //Go up
            ...
            else if(curMove === 97 || curMove === 65){    //Go left
            ...
            else if(curMove === 68 || curMove === 100){     //Go right
            ...
            else if(curMove === 115 || curMove === 83){     //Go down
                if(snakeBody[snakeLength-1] >= boardSize*boardSize-boardSize){
                    lastHead += boardSize-boardSize*boardSize
                }
                else{
                    lastHead += boardSize
        ...
        $('.game-time').click(() => {
            $('.game-time').off('click')
            $('.score').html('')
            $('.game-time').text('Playing... (use w, a, d, s to move)').addClass('playing')
            ...
            $(document).bind('keypress', function (event) {
                if((curMove === 119 || curMove === 87) && (event.keyCode === 97 || event.keyCode === 65 ||
                    event.keyCode === 68 || event.keyCode === 100)){
                        curMove = event.keyCode
                }
                if((event.keyCode === 97 || event.keyCode === 65 || event.keyCode === 68 || event.keyCode === 100) && 
                    (curMove === 115 || curMove === 83)){
                        curMove = event.keyCode
                }
                if((curMove === 97 || curMove === 65) && 
                    (event.keyCode === 119 || event.keyCode === 87 || event.keyCode === 115 || event.keyCode === 83)){
                        curMove = event.keyCode
                }
                if((curMove === 68 || curMove === 100) && 
                    (event.keyCode === 119 || event.keyCode === 87 || event.keyCode === 115 || event.keyCode === 83)){
                        curMove = event.keyCode
                }
            })
