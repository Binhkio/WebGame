    ...
    $('.memory-game').click(()=>{
        ...
        function checkMatch(){
            ...
            if(chosenCard[card1].name === chosenCard[card2].name){
                ...
                $('#' + (chosenCardId[card1]+1) + '>img , #' + (chosenCardId[card2]+1) + '>img').animate({opacity:0},500)
            } else{
                current_score -= 10
                imgList[chosenCardId[card1]].chosen = false
                imgList[chosenCardId[card2]].chosen = false
                setTimeout(()=>{
                    $('#'+(chosenCardId[card1]+1)).html('<img src="img/blank.png">')
                    $('#'+(chosenCardId[card2]+1)).html('<img src="img/blank.png">')
                },500)
            }
                ...
                $.each($('.square'), function (indexInArray) { 
                    $(this).click(function(){
                        if(imgList[indexInArray].chosen === false){
                            ...
                            $(this).html('<img src="' + imgList[indexInArray].source + '">')
